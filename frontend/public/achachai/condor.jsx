...
/* ============================================================
   CONDOR — anatomical SVG with independently animated wings,
   head, beak, eye, tail and feet. Five moods:
     idle  — gentle soaring, wings ride thermals
     think — wings flap slowly + head bob
     speak — head tilts, beak opens, body breathes
     alert — full dive: rotates, wings tucked, sonar rings + red glow
     still — no animation
   Tones: wing (default deep), marfil (light on dark),
...
function Condor({ size = 32, mood = "idle", tone = "wing", className = "", style = {}, sonar = false }) {
  const c = TONE[tone] || TONE.wing;

  /* per-mood orchestration */
  const bodyAnim = {
  ...
  return (
    ...
    >
      ...
      <g style={{ transformOrigin: "40px 50px", animation: bodyAnim }}>
        ...
        {/* LEFT WING — pivots at shoulder (28,46) */}
        <g style={{ transformOrigin: "28px 46px", animation: wingLAnim }}>
          ...
          <path d="M28 46
                   ...
                   C 6 52, 16 50, 24 54
                   C 18 56, 12 60, 8 66
                   C 18 60, 26 56, 30 54 Z"
                fill={c.body}/>
          {/* secondary upper covert */}
          <path d="M28 46 C 18 40, 8 42, 2 50 C 12 46, 22 46, 28 48 Z"
        ...
        {/* RIGHT WING — pivots at shoulder (52,46) */}
        <g style={{ transformOrigin: "52px 46px", animation: wingRAnim }}>
          <path d="M52 46
                   C 66 38, 76 44, 84 56
                   C 74 52, 64 50, 56 54
                   C 62 56, 68 60, 72 66
                   C 62 60, 54 56, 50 54 Z"
                fill={c.body}/>
          <path d="M52 46 C 62 40, 72 42, 78 50 C 68 46, 58 46, 52 48 Z"
                fill={c.accent} opacity="0.85"/>
        ...
        {/* COLLAR — Andean condor's white ruff (signature feature) */}
        ...
        {/* HEAD GROUP — pivots at base of neck (40,38) */}
        <g style={{ transformOrigin: "40px 38px", animation: headAnim }}>
          ...
          {/* head crest (caruncle/bare head texture) */}
...
/* tiny version for chips/lists — minimal silhouette only */
function CondorMini({ size = 14, tone = "wing" }) {
  const c = TONE[tone] || TONE.wing;
  return (
...
/* condor over peaks — header decoration */
function CondorOverPeaks({ width = 120, mood = "idle" }) {
  return (
    <svg viewBox="0 0 220 90" width={width} style={{ display: "block" }}>
      ...
      <path d="M0 80 L30 45 L55 65 L95 25 L135 60 L170 40 L220 65 L220 90 L0 90 Z"
            fill="#2C5F8D" opacity="0.20"/>
      <path d="M0 88 L40 60 L80 75 L120 52 L160 70 L220 60 L220 90 L0 90 Z"
            fill="#1A3A52" opacity="0.18"/>
...
/* ============================================================
   VUELO DEL CÓNDOR — score component
   ============================================================ */
function VueloDelCondor({ score = 14, variant = "md", label, sublabel, signals = [] }) {
  const level = score >= 70 ? "red" : score >= 40 ? "amber" : "green";
  const palette = {
    red:   { ring: "#C5333A", soft: "rgba(197,51,58,0.16)", text: "Descenso",     desc: "Revisión inmediata", anim: "pulse-red 1s infinite",   mood: "alert", tone: "red"   },
    amber: { ring: "#D4A574", soft: "rgba(212,165,116,0.20)", text: "Observación", desc: "Requiere revisión",  anim: "pulse-amber 2s infinite", mood: "think", tone: "wing"  },
    green: { ring: "#4A7C59", soft: "rgba(74,124,89,0.16)",  text: "Vuelo alto",   desc: "Todo en calma",      anim: "pulse-green 4s infinite", mood: "idle",  tone: "wing"  },
  }[level];
    ...
    <div style={{ display: "inline-flex", flexDirection: "column", alignItems: "center", gap: variant === "sm" ? 4 : 8 }}>
      <div style={{
        ...
        <svg width={S.ring} height={S.ring} style={{ position: "absolute", inset: (S.box - S.ring) / 2, transform: "rotate(-90deg)" }}>
          <circle cx={S.ring / 2} cy={S.ring / 2} r={S.ring / 2 - 6}
                  fill="none" stroke="rgba(26,58,82,0.08)" strokeWidth="3"/>
          <circle cx={S.ring / 2} cy={S.ring / 2} r={S.ring / 2 - 6}
                  fill="none" stroke={palette.ring} strokeWidth="3"
        ...
        {level === "red" && variant !== "sm" && (
          <div style={{
            ...
            border: "2px solid rgba(197,51,58,0.30)",
...
/* ============================================================
   CÓNDOR BUBBLE — floating widget, always present
   ============================================================ */
function CondorBubble({ onOpen, mood = "idle", message }) {
    ...
    <div style={{ position: "fixed", bottom: 24, right: 24, zIndex: 80, display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 10 }}>
      ...
      <button
        ...
        style={{
          ...
          border: "3px solid var(--marfil-paper)",
...
/* ============================================================
   AUTO-APRENDIZAJE BAR
   ============================================================ */
function LearningBar({ count = 47, delta = 2.3 }) {
  return (
    <div style={{
...
/* expose globally */
Object.assign(window, { Condor, CondorMini, VueloDelCondor, CondorBubble, LearningBar, CondorOverPeaks });
